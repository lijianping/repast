...
{
	Initialize();
	SQLRETURN m_return_code;   // sql 返回类型
	// 绑定返回值
	m_return_code_ = SQLBindParameter(m_hstmt_, 1, SQL_PARAM_OUTPUT, SQL_C_SSHORT, SQL_INTEGER,\
		                       0, 0, &m_pro_ret, 0, &m_sql_pro_ret);
	if (m_return_code != SQL_SUCCESS && m_return_code != SQL_SUCCESS_WITH_INFO) 
		LTHROW(BIND_RETURN_ERROR)
	m_return_code = SQLBindParameter(m_hstmt_, 2, SQL_PARAM_INPUT, SQL_CHAR, SQL_C_CHAR, sizeof(m_main_name_) - 1,\
		                       0, m_main_name_, sizeof(m_main_name_), &m_sql_main_name_);
	if (m_return_code != SQL_SUCCESS && m_return_code != SQL_SUCCESS_WITH_INFO) 
		LTHROW(BIND_PARAM_ERROR)
	m_return_code = SQLBindParameter(m_hstmt_, 3, SQL_PARAM_INPUT, SQL_CHAR, SQL_C_CHAR, sizeof(m_child_name_) - 1,\
	...
	m_return_code = SQLBindCol(m_hstmt_, 1, SQL_C_CHAR, m_name_, sizeof(m_name_), &m_sql_name_);
...
void CCommodity::Initialize() {
	...
	memset(m_name_, 0, sizeof(name_));
	memset(m_main_name_, 0, sizeof(main_name_));
	memset(m_child_name_, 0, sizeof(child_name_));
}